...
int APIENTRY _tWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPTSTR lpCmdLine, _In_ int nCmdShow)
{
    ...
    if (!bVFSEnabled && !(flag & DISABLEVFS)) {
        ...
        if (!FileExists(expandedPath)) {
            ...
            //MessageBox(0, L"AutoHotkey wasn't found in the AppData folder!", L"Error", MB_OK | MB_ICONERROR);
            return 0;
        }
        ...
        if (lpExitCode && !(flag & LAUNCH)) {
            HANDLE hChild = OpenProcess(SYNCHRONIZE, false, lpExitCode);
            if (hChild) {
                lpExitCode = 0;
                WaitForSingleObject(hChild, INFINITE);
                GetExitCodeProcess(hChild, &lpExitCode);
                CloseHandle(hChild);
            }
        }
        return lpExitCode;
    ...
    if (flag & RUNAS) { // Run this same exe with all arguments after "/runas" as admin
        ConcatArgs(wstrRemainder, argv, pNumArgs, startPoint);
        OutputDebugString(szFileName);
        CoInitializeEx(NULL, COINIT_APARTMENTTHREADED | COINIT_DISABLE_OLE1DDE);
        ShellExecute(0, L"runas", szFileName, wstrRemainder.c_str(), 0, 0);
        CoUninitialize();
        return 0;
    }
    ...
    CreateProcess(NULL,   // No module name (use command line)
        ...
        &si,            // Pointer to STARTUPINFO structure
        &pi);           // Pointer to PROCESS_INFORMATION structure
    childPID = GetProcessId(pi.hProcess);

    // When the /launch switch is used, return the process ID as the launcher's exit code.
    // Unless /launch was injected into the next command, in which case wait for termination
    // of the launcher and return the launched process' ID
    if ((flag & LAUNCH) && !bInjectedLaunch) {
        CloseHandle(pi.hProcess);
        CloseHandle(pi.hThread);
        return childPID;
    }
    ...
    CloseHandle(pi.hProcess);
    CloseHandle(pi.hThread);
    if (!(flag & LAUNCH) && bInjectedLaunch && lpExitCode) {
        HANDLE hChild = OpenProcess(SYNCHRONIZE, FALSE, lpExitCode);
        if (hChild) {
            WaitForSingleObject(hChild, INFINITE);
            lpExitCode = 0;
